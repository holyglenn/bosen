...
hotbox_projdir = join(project_dir, 'third_party', 'include', 'hotbox')
dataset_dir = join(project_dir, 'app', 'mlr', 'datasets')
sys.path.append(join(project_dir, 'third_party', 'include', 'hotbox', 'build'))
...
class WarpClient:
  def __init__(self):
    ...
    with open(join(hotbox_projdir, 'config.yaml')) as f:
...
if __name__ == "__main__":
  ...
  test_db = db_client.CreateDB('bosen_db')
  test_db.ReadFile('resource/dataset/a1a.toy',
      file_format='libsvm')
  #test_db.ReadFile(dataset_dir + '/covtype.scale.train.small',
  #    file_format='libsvm')
